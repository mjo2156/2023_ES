...
int main()
{
  ...
  for (;;) {
	...
	if (write(sockfd, &buffer_out_char, count) < 0){
        	printf("error");
        } else {
      ...
      if ((packet.modifiers == 0x02) || (packet.modifiers == 0x20) || (packet.modifiers == 0x22)) // check for other shift cases
      {
        switch(packet.keycode[0]){
          case 0x04: // Keyboard a 
            buffer_out_char[count] = 'A';
            sprintf(buffer,"A");
            break;
...
void *network_thread_f(void *ignored)
{
  ...
  int n;
  /* Receive data */
  while ( (n = read(sockfd, &recvBuf[8], BUFFER_SIZE - 1)) > 0 ) {
    recvBuf[8][n-1] = '\0';
